// =====================================================================
// 🕹️ INYECCIÓN DE ESTILOS CSS PARA MODALES ARCADE PERSONALIZADOS
// =====================================================================
const estilosArcade = document.createElement('style');
estilosArcade.innerHTML = `
    .arcade-modal-overlay {
        position: absolute; top: 0; left: 0; width: 100%; height: 100%;
        background: rgba(0, 0, 0, 0.75); display: flex; align-items: center;
        justify-content: center; z-index: 9999; font-family: 'Press Start 2P', monospace;
    }
    .arcade-modal {
        background: #110d26; border: 4px double #ffffff; padding: 25px;
        width: 380px; text-align: center; box-shadow: 0 0 15px #b03b7b;
    }
    .arcade-title { color: #ffeb5c; font-size: 14px; margin-bottom: 15px; text-transform: uppercase; }
    .arcade-msg { color: #ffffff; font-size: 10px; margin-bottom: 20px; line-height: 1.6; }
    .arcade-input {
        width: 85%; background: #000000; border: 2px solid #b03b7b; padding: 10px;
        color: #6bff6b; text-align: center; font-family: 'Press Start 2P';
        font-size: 12px; outline: none; text-transform: uppercase; margin-bottom: 20px;
    }
    .arcade-btn-container { display: flex; justify-content: space-around; }
    .arcade-btn {
        background: #1a153a; border: 2px solid #ffffff; color: #ffffff;
        padding: 8px 15px; font-family: 'Press Start 2P'; font-size: 10px; cursor: pointer;
    }
    .arcade-btn:hover { background: #b03b7b; color: #ffeb5c; }
`;
document.head.appendChild(estilosArcade);
...
// =====================================================================
// 🌙 CONFIGURACIÓN DEL MOTOR PHASER (MOON ARCADE COMPLETE PRO)
// =====================================================================
const config = {
    type: Phaser.AUTO,
    width: 800,
...
// =====================================================================
// 💾 GESTOR DE HISTORIAL DE PUNTUACIONES
// =====================================================================
const GestorScores = {
    obtenerTopScores: function() {
        let scores = localStorage.getItem('moon_arcade_leaderboard');
        return scores ? JSON.parse(scores) : [
            { name: 'AYLIN', score: 300 },
            { name: 'NACHO', score: 250 },
            { name: 'EVAN', score: 150 },
            { name: 'GABO', score: 120 },
            { name: 'RICHI', score: 90 }
        ];
    },
    registrarNuevoScore: function(nombre, puntuacion) {
        let lista = this.obtenerTopScores();
        lista.push({ name: nombre.substring(0, 6).toUpperCase(), score: puntuacion });
        lista.sort((a, b) => b.score - a.score);
        lista = lista.slice(0, 7); 
        localStorage.setItem('moon_arcade_leaderboard', JSON.stringify(lista));
    }
};
...
GameScene.prototype.create = function() {
    ...
    // 🌙 LUNA RESTAURADA EN EL GAMEPLAY
    const luna = this.add.graphics().setDepth(0);
    luna.fillStyle(0xffe57f, 1).fillCircle(120, 100, 40);
    luna.fillStyle(0x1a153a, 1).fillCircle(135, 85, 40); 

    // ✨ MÁS ESTRELLAS: Aumentadas a 85 con parpadeo desfasado
    for (let i = 0; i < 85; i++) {
    ...
    // ☁️ GENERADOR PROCEDURAL DE NUBES DE 8-BITS ANIMADAS (De Derecha a Izquierda)
    if (!this.textures.exists('nube_arcade')) {
        let g = this.make.graphics({ x: 0, y: 0, add: false });
        g.fillStyle(0xffffff, 0.35); // Opacidad retro suave
        g.fillRect(30, 0, 70, 14);
        g.fillRect(15, 14, 100, 14);
        g.fillRect(0, 28, 130, 14);
        g.generateTexture('nube_arcade', 130, 42);
    }

    this.nubesGrupo = this.add.group();
    const configsNubes = [
        { x: 200, y: 140, speed: -0.4 },
        { x: 550, y: 70,  speed: -0.7 },
        { x: 850, y: 190, speed: -0.3 }
    ];
    configsNubes.forEach(c => {
        let n = this.add.image(c.x, c.y, 'nube_arcade').setOrigin(0.5).setScale(1.5).setDepth(0);
        n.setData('speedX', c.speed);
        this.nubesGrupo.add(n);
    });
    ...
    this.platforms.add(floorCollider);

    // Urban landscape & layers
    ...
    this.add.image(130, PISO_Y + 55, 'arbol_rosa1').setOrigin(0.5, 1).setScale(3.2).setDepth(1);
    this.add.image(670, PISO_Y + 55, 'arbol_rosa3').setOrigin(0.5, 1).setScale(3.2).setDepth(1);
    ...
    if (!this.anims.exists('run')) {
        ...
        this.anims.create({ key: 'jump', frames: this.anims.generateFrameNumbers('aylin', { start: 24, end: 31 }), frameRate: 10, repeat: 0 });
    }
    ...
    menuBtn.on('pointerdown', () => {
        if(musicaFondo) { musicaFondo.stop(); musicaFondo = null; } 
    ...
    if (this.sys.game.device.input.touch && !this.sys.game.device.os.desktop) {
        ...
        btnIzq.on('pointerdown', () => this.touchLeft = true);
        btnIzq.on('pointerup', () => this.touchLeft = false);
        btnIzq.on('pointerout', () => this.touchLeft = false);
        ...
        btnDer.on('pointerdown', () => this.touchRight = true);
        btnDer.on('pointerup', () => this.touchRight = false);
        btnDer.on('pointerout', () => this.touchRight = false);
        ...
        btnJump.on('pointerdown', () => {
            if (this.player.body.touching.down) this.player.setVelocityY(-460);
        });
    }
...
GameScene.prototype.update = function() {
    // ☁️ DESPLAZAMIENTO CONTINUO DE NUBES DE DERECHA A IZQUIERDA
    this.nubesGrupo.getChildren().forEach(nube => {
        nube.x += nube.getData('speedX');
        if (nube.x < -100) {
            nube.x = 900; // Warp back to the right edge seamlessly
            nube.y = Phaser.Math.Between(50, 220); // Cambia de altura al azar
        }
    });
    ...
    this.fallingItems.getChildren().forEach(item => {
        if (item.y > 620) item.destroy();
    });
};

GameScene.prototype.spawnItem = function() {
    const pool = [
        { key: 'uno', points: 10, scale: 2.2 },
        { key: 'gomita', points: 15, scale: 1.8 },
        { key: 'zorro', points: 20, scale: 2.5 },
        { key: 'bts', points: 25, scale: 1.5 },
...
GameScene.prototype.catchObject = function(player, item) {
    const value = item.getData('pointsValue');
    this.score += value;
...
GameScene.prototype.nextLevelTransition = function() {
    ...
    this.score = 0; 
    this.timeLeft = 45; 
    this.targetScore = 100 + (this.currentLevel * 25); 
...
WinScene.prototype.init = function(data) {
    ...
    this.levelReached = data.levelReached || 1;
};
...
WinScene.prototype.create = function() {
    ...
    btnRegistrar.on('pointerdown', () => {
        abrirPromptArcade('⌨️ NUEVA MARCA', 'Ingresa tus iniciales para el Top (Max 6 letras):', (nick) => {
            if (nick && nick.trim() !== "") {
                GestorScores.registrarNuevoScore(nick, this.finalScore);
    ...
    btnSecreto.on('pointerdown', () => {
        abrirPromptArcade('🔒 BÓVEDA', 'Introduce el código', (llave) => {
            if (llave && llave.toLowerCase().trim() === '4yl1n007') {
...
LeaderboardScene.prototype.create = function() {
    ...
    let scores = GestorScores.obtenerTopScores();
...
// =====================================================================
// 🎬 ESCENA OCULTA 5: PANTALLA DE CUMPLE (CÓDIGO MASTER: 4yl1n007)
// =====================================================================
...
BirthdayScene.prototype.create = function() {
    ...
    for (let i = 0; i < 20; i++) {
        let tx = Phaser.Math.Between(50, 750);
        let ty = Phaser.Math.Between(50, 550);
    ...
    // Dedicatoria y texto de la carta exactamente como lo pediste
    let textoCarta = 
        ...
        "llena de cosas positiivas y sobre todo de mucha paz y amor en tu corazon.\n" +
        "Feliz vuelta al sol.🌞\n\n" +
        "Tqm Moon.🌙\n\n" +
        "- Ivan.";
    ...
    const salirOculto = this.add.text(400, 540, '< CERRAR SISTEMA >', { fontFamily: '"Press Start 2P"', fontSize: '9px', fill: '#555555' }).setOrigin(0.5).setInteractive();
